...
const SignupForm = ({setIsSucceed}) => {
  ...
  const userSignup = async () => {
    const response = await axios.post("http://localhost:3000/v1/user/signup" , data);
    console.log(response.data);
    setIsSucceed(response.data.success);
  }

  useEffect(()=>{
    if(data) (async () => { await userSignup()})();
  },[data]);
  ...
  return (
    ...
    >
      ...
      <button type="submit" className="cursor-pointer w-full py-3 rounded-lg bg-cta-btn">
        Create Account
      </button>
    </form>
  );
